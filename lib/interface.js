...
export default class RaftInterface extends Emitter {
    ...
    sendAppendEntries(node, prevLogIndex) {
        ...
        // that one.
        logIndex = Math.max(logIndex, this.minimumIndex);
        logIndex = Math.min(logIndex, this.logIndex);
        ...
        return new Promise((resolve, reject) => {
            ...
            var bindRPC = () => {
                rpcCall.then(success => {
                    ...
                    logIndex--;
                    if (logIndex < this.minimumIndex) logIndex = Math.min(this.minimumIndex, this.logIndex);
...
function bindRPC(raft) {
    raft.rpcHandlers.set('AppendEntries', (args, leaderID, term) => {
        ...
        if (args.prevLogIndex !== -1 && args.prevLogIndex < raft.minimumIndex) {
            // Some caching
        ...
        // whose term matches prevLogTerm (§5.3)
            ...
            args.prevLogIndex !== -1 && // Ignore non-existant logs that don't exist
            !raft.logEntryAt(args.prevLogIndex)) {
            return false;
        }
        ...
        if (args.leaderCommit > raft.commitIndex) {
            raft.setCommitIndex(
                Math.min(args.leaderCommit, args.prevLogIndex + args.entries.length)
            );
        }
